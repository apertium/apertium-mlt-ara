...
if [ "$4" == "update" ]; then
    ...
    wget -O $TMPHTML -q https://wiki.apertium.org/wiki/Maltese_and_Arabic/$TESTTYPE
    if [[ -s $TMPHTML ]]; then mv $TMPHTML $HTML;
    else rm $TMPHTML; echo "Couldn't fetch https://wiki.apertium.org/wiki/Maltese_and_Arabic/$TESTTYPE"; fi
fi
echo "..."
